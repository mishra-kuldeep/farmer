...
const Basket = () => {
  ...
  const totalDiscount = cart?.cart?.reduce((acc, item) => {
    const discount = item?.productDetail?.discountType === "fixed"
      ? item?.productDetail?.discount * item?.quantity
      : (item?.productDetail?.price * item?.productDetail?.discount / 100) * item?.quantity;
    return acc + discount;
  }, 0);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const response = await OrderService.checkoutCart(checkoutData) // Replace with your API endpoint
