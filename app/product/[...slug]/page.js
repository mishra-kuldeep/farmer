...
const Product = () => {
  ...
  const handleGetProduct = async () => {
    try {
      const singleProducthResult = await ProductsDtlServices.getsingleProductsDtl(slug);
    ...
    <div className="container">
      ...
      <div className="product_basic_detail">
        ...
        <div className="product_details">
          <h3>{sigleSearchProduct?.productDtlName}</h3>
          <p>MRP: <del>₹34.25</del></p>
          <h6 className="fw-bold">Price: {sigleSearchProduct?.price}</h6>
          <h6 className="fw-bold">unit: {sigleSearchProduct?.unit}</h6>
